node() {
    ...
    stage('Run Tests') {
        try {
            sh('SKIP_IO_CAPTURE_TESTS=1 NO_INTERACTION=1 TEST_PHP_JUNIT=src-junit.xml REPORT_EXIT_STATUS=1 ./php-install/bin/php run-tests.php -P -g "FAIL,XFAIL,BORK,WARN,LEAK,SKIP" --show-diff');
        }
        finally {
            junit('*junit.xml');
        }
