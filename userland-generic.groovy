node() {
    ...
    stage('Run Composer Install') {
        ...
        sh("touch php.ini");
        sh("COMPOSER_ROOT_VERSION=${composerRootVersion} php-install/bin/php -c php.ini composer.phar install --no-interaction --no-progress");
    }

    stage('Run Tests') {
        try {
            wrap([$class: 'AnsiColorBuildWrapper']) {
                sh("php-install/bin/php -c php.ini ${PHPUNIT_PATH} --exclude-group tty,benchmark,intl-data,slow,legacy --log-junit=tests-junit.xml");
            }
        }
        finally {
            junit('*junit.xml');
